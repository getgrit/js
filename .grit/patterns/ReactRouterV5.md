...
# Upgradee React Router v5 to v6

```grit
language js

pattern handle_redirect_switch() {
  ReactNode(name=`Redirect`, props=$props) as $redirect => `<Route path='$from' render={() => <Redirect $props />} />` where {
      $props <: contains { `from="$from"` => . }
  }
}

pattern rename_switch() {
  ReactNode(name=`Switch` => `Routes`, children=$children) where {
      $children <: maybe contains handle_redirect_switch()
  }
}
